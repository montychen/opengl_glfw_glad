...
** 模型矩阵Model matrix： 模型变换包括 S缩放、R旋转、T移动
#+begin_quote
...
#+begin_quote
TransformedVector = TMatrix * RMatrix * SMatrix * OriginalVector;  # opengl矩阵的运算顺序是从右往左进行，在最右边的矩阵是第一个与向量相乘的
#+end_quote
...
- 创建视图矩阵: Matrix4.setLookAt(eyeX, eyeY, eyeZ, atX, atY, atZ, upX, upY, upZ) 创建视图矩阵
  - eyeX,eyeY, eyeZ: 指定视点
  - atX, atY, atZ: 观察目标点
  - upX, upY, upZ: 指定上方向
