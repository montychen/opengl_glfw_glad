...
* 向量
- 向量最基本的定义就是一个方向。或者说，向量有一个方向(Direction)和大小(Magnitude，也叫做强度或长度)。 *由于向量表示的是方向，起始于何处并不会改变它的值* 。下图可以看到向量v¯和w¯是相等的，尽管他们的起始点不同。
#+ATTR_latex: :width 300   #+ATTR_HTML: :width 300  #+ATTR_ORG: :width 300
[[file:webgl/vectors.png]]
- 由于向量是一个方向，很难用位置(Position)表示出来。为了直观，通常设定方向的原点为(0, 0, 0)，然后指向一个方向，对应一个点，使其变为位置向量(Position Vector)。比如位置向量(3, 5)在图像中的起点是(0, 0)，并指向(3, 5)
...
** 向量加减
#+begin_quote
向量的加法满足平行四边形法则和三角形法则. 具体地，两个向量 a⃗和b⃗相加，得到的是另一个新向量. 这个新向量可以表示为a⃗和b⃗的起点重合后，以它们为邻边构成的平行四边形的一条对角线（以共同的起点为起点的那一条，见下图左）
或者表示为将 a⃗的终点和 b⃗的起点重合后，从a⃗的起点指向 b⃗的终点的这一新向量：
#+end_quote
...
var FSHADER_SOURCE =
    ...
    'precision mediump float;\n' +
    '#endif\n' +
    'uniform sampler2D u_Sampler0;\n' + // 在同一个几何体平面上同时使用多个纹理，所以这里定义了两个纹理采样器
    'uniform sampler2D u_Sampler1;\n' + // sampler2D类型对应gl.TEXTURE_2D； samplerCube类型对应gl.TEXTURE_CUBE_MAP
    'varying vec2 v_TexCoord;\n' +
    'void main() {\n' +
