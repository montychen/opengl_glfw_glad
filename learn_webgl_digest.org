...
* 基本概念
- *光栅（Raster）* ： 由像素构成的一个矩形网格。要在光栅上显示的数据保存于帧缓存内FrameBuffer
...
- 叉乘v × k，在3D空间中有定义，它需要两个不平行向量作为输入，生成一个正交于两个输入向量的 *法向量* 。如果输入的两个向量也是正交的，那么叉乘之后将会产生3个互相正交的向量
  - 根据叉乘的结果，还可以判断一个向量是在另一个向量的左边，还是右边。 更进一步，还可以利用叉乘的结果，来判断一个点是否在一个三角形内。
#+ATTR_latex: :width 200   #+ATTR_HTML: :width 200  #+ATTR_ORG: :width 200
...
- 点乘v ⋅ k = |v||k|cosθ = cosθ  (如果v¯和k¯都是单位向量，它们的长度会等于1), 点乘的结果是一个标量，可以计算向量v和k之间的夹角θ
  ...
  - 点乘的几何意义是：是一条边向另一条边的投影乘以另一条边的长度: v.k 是k在v上面的投影
...
*  test org table 中英文是否对齐
| aa      | name               | age        | note     | 备注   |
| 不妥bbb | 带我看迪塞尔落地尅 | addaiekdai | 借贷而开 | 开发贷 |
| 测试    |                    |            |          | aaa    |
| 陈卓仪  | sa硫报价           | dasi       | 30       | kkkkkk |
| ]       |                    |            |          |        |
