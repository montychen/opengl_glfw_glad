...
glm::vec4 transformedVector = myMatrix * myVector; // 记住: 矩阵左乘顶点
...
vec4 myVector;
// fill myMatrix and myVector somehow
vec4 transformedVector = myMatrix * myVector; // 实际中我们不用GLSL计算变换矩阵, 在C++中用glm::translate()算出矩阵，然后把它传给GLSL。在GLSL中只做一次乘法.
#+end_src
...
** 累积变换（缩转移）：在组合矩阵时，正确的顺序是：先缩放、再旋转、最后才是平移，否则它们会（消极地）互相影响。
#+begin_quote
...
* 相机视角&视图矩阵view matrix：视点eye point、观察目标点look-at point、上方向up direction
#+begin_quote
一个物体要在OpenGL中被渲染出现，需要经过经典的 *MVP变换* :
1. 从物体坐标系，通过Model Matrix，变换到世界坐标系.
2. 然后通过View Matrix，将顶点从世界坐标系转到到相机坐标系, 表示观察世界的方式。
3. 最后通过projection Matrix，做仿射变换，最后还有一步OpenGL隐藏的剪裁步骤，裁减掉不在投影区域内的像素点。
#+end_quote
#+ATTR_latex: :width 500   #+ATTR_HTML: :width 500  #+ATTR_ORG: :width 500
...
** 视图矩阵View Matrix：将顶点从世界坐标系转到到相机坐标系下的变化矩阵。通过视点、观察目标点、上方向创建视图矩阵
#+begin_quote
Matrix4.setLookAt(eyeX, eyeY, eyeZ, atX, atY, atZ, upX, upY, upZ) 创建视图矩阵
- eyeX,eyeY, eyeZ: 指定视点
- atX, atY, atZ: 观察目标点
- upX, upY, upZ: 指定上方向，
#+end_quote
* 模型视图矩阵model view matrix
- 模型视图矩阵 = 视图矩阵 ✖️ 模型矩阵    #模型矩阵指的是：缩放矩阵、者旋转矩阵、平移矩阵
- 模型矩阵的累积变换（缩转移）：在组合矩阵时，正确的顺序是：先缩放、再旋转、最后才是平移，否则它们会（消极地）互相影响。
- TransformedVector = TranslationMatrix * RotationMatrix * ScaleMatrix * OriginalVector;
