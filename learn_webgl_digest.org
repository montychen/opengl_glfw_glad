...
- 不论图片尺寸有多大，长和宽各是多少，纹理坐标总是从0到1之间取值。 
...
2. 配置纹理
   ...
   7. 把纹理图像分配给纹理对象  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, image)  把jpg纹理图像gl.RGB颜色格式，分配给2D的纹理对象。
