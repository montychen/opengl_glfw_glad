...
** 累积变换（缩旋移）：在组合矩阵时，正确的顺序是：先缩放、再旋转、最后才是平移，否则它们会（消极地）互相影响。
#+begin_quote
...
* 相机视角：视点eye point、观察目标点look-at point、上方向up direction
#+begin_quote
一个物体要在OpenGL中被渲染出现，需要经过经典的 *MVP变换* :
1. 主要是从物体坐标系，通过Model Matrix，变换到世界坐标系.
2. 然后通过View Matrix，也叫相机坐标系，转换为某个视角所看到的内容.
...
** 视图矩阵View Matrix：将顶点有世界坐标系转到到相机坐标系下的变化矩阵。通过视点、观察目标点、上方向来创建出视图矩阵
#+begin_quote
Matrix4.setLookAt(eyeX, eyeY, eyeZ, atX, atY, atZ, upX, upY, upZ) 创建出视图矩阵
- eyeX,eyeY, eyeZ: 指定视点
- atX, atY, atZ: 观察目标点
- upX, upY, upZ: 指定上方向，
#+end_quote
