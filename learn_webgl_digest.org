...
- 纹理贴图的作用就是根据纹理图片，为光栅化后的片元涂上合适的颜色。组成纹理图像的像素又被称为 *纹素(texels, texture elements)*, 每一个纹素的颜色都是RGB或RGBA格式。如图：
#+ATTR_latex: :width 700   #+ATTR_HTML: :width 700  #+ATTR_ORG: :width 700
[[file:webgl/texel.png]]

** 为了实现纹理贴图我们需要做3件事：
1. 将一张贴图加载到OpenGL中
2. 提供纹理坐标和顶点（将纹理对应匹配到顶点上）
...
** 纹理贴图，需要4步：
1. 准备好图片，准备贴到几何图形上的图片。
2. 配置映射方式，使用纹理坐标texture coordinates将纹理图像映射到几何图形上。
3. 加载纹理图像
4. 抽取纹素，把颜色赋给片元，在片元着色器中，将相应的纹素从纹理中抽取出来，并将纹素的颜色赋给片元。


** 纹理贴图需要顶点着色器和片元着色器二者软的配合：
1. 首先在顶点着色器中将纹理坐标映射到顶点上。通过纹理图像的纹理坐标与几何体的顶点坐标间的映射关系，来确定将那些纹理图像贴上去。app传入顶点坐标和对应的纹理坐标
#+ATTR_latex: :width 700   #+ATTR_HTML: :width 700  #+ATTR_ORG: :width 700
[[file:webgl/texture_vertex_map.png]]
1. 然后在片元着色器中，根据每个片元的纹理坐标从纹理图像中抽取纹理颜色，赋给当前片元

   创建纹理和创建顶点缓冲差不多：创建一个纹理、绑定、填充、配置。

2. 反转纹理图像的y轴。WebGL的纹理坐标系统的t轴方向和PNG、JPG等图片格式的坐标系统的Y轴方向相反，所以要先将图像Y轴反转，方便我们操作坐标。 gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 1)
...
3. 激活纹理单元    gl.activeTexture(gl.TEXTURE0)  激活0号纹理单元·
...
4. 把纹理对象绑定到目标区域  gl.bindTexture(gl.TEXTURE_2D, texture)   webgl支持两种类型的纹理 gl.TEXTURE_2D 二维纹理;  gl.TEXTURE_CUBE_MAP 立方体纹理

5. 配置纹理对象的参数  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR)
   - gl.texParameteri(target, pname, param) ，将param的值赋给绑定到目标的纹理对象的pname参数上。参数：
     - 第1个参数target： 指定纹理的类型，gl.TEXTURE_2D二维纹理  或者 gl.TEXTURE_CUBE_MAP立方体纹理
     - 第2个参数pname：纹理参数的名字，决定了获取纹素颜色的方式；
       - 放大方法：gl.TEXTURE_MAG_FILTER，表示当纹理的绘制范围比纹理本身更大时，获取纹素颜色的方式；比如说：你将16*16的纹理图像映射到32*32像素的空间时，纹理的尺寸就变成了原来的两倍，被放大了，该参数决定了如何填充这些放大的空隙部分。
       - 缩小方法：gl.TEXTURE_MIN_FILTER，表示当纹理的绘制范围比纹理本身小的时，获取纹素颜色的方式；比如说：你将32*32的纹理图像映射到16*16像素的空间时，纹理的尺寸就只有原来的一半，变小了，需要剔除纹理图像中的部分像素。该参数决定了剔除的方法。
       - 水平填充方法：gl.TEXTURE_WRAP_S， 表示如何对纹理图像左侧或者右侧的区域进行填充；
   ...
   6. 分配给纹理对象  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, image); // 配置纹理图像
