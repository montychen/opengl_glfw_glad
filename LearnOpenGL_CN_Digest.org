...
** 投影: 使三维空间的物体变二维, 投影矩阵干了两件事: 裁剪clipping(即frustum culling，视椎体剔除) + 生成NDC。
*投影矩阵(projection matrix)* :计算机的显示器是二维的, 一个3D场景需要被投影到屏幕上成为一个2D图像，这称为投影变换，需要用到投影矩阵。
1. 首先，投影矩阵会把所有顶点坐标从eye coordinates(观察空间)变换到 *裁剪坐标(clip coordinate，属于裁剪空间，clip space)* 。
2. 然后，这些裁剪坐标被变换到标准化设备坐标(normalized device coordinates, NDC，即坐标范围在-1到1之间)，这一步是通过用裁剪坐标的 分量除裁剪坐标实现的。
因此，我们要记住 *投影矩阵干了两件事: 裁剪clipping(即frustum culling，视椎体剔除) + 生成NDC* 。
...
* OpenGL函数
** glLoadIdentity()，glPushMatrix()，glPopMatrix() - glLoadIdentity()的作用就是把当前矩阵设为为单位矩阵. - glPushMatrix、glPopMatrix相当于堆栈里的入栈和出栈。 *Push起到保护环境、Pop起到恢复环境的作用* 。这2个函数可以嵌套使用。调用glPushMatrix就是把 当前矩阵做一个副本放入堆栈，然后不管你之后做了多少变换，这时调用glPopMatrix，当前矩阵就可以恢复到你调用glPushMatrix之前的那个状态。例如当前的 坐标系原点在电脑屏幕的左上方。现在调用glPushMatrix，然后再调用一堆平移、旋转代码等等，然后再画图。那些平移和旋转都是基于坐上角为原点进行变化的。 而且都会改变坐标的位置，经过了这些变化后，你的坐标肯定不再左上角了。如果想恢复怎么办？这时调用glPopMatrix从栈里取出一个“状态”，这个状态就是 你调用glPushMatrix之前的那个状态。
