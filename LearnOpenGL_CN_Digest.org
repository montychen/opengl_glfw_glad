...
着色器(Shader)是运行在GPU上的小程序，它们之间不能相互通信；它们之间唯一的沟通只有通过输入和输出。着色器的开头总是要声明版本，接着是输入和输出变量inout变量、uniform和main函数。每个着色器的入口点都是main函数，在这个函数中我们处理所有的输入变量（用in关键字声明的变量），并将结果输出到输出变量中（用out关键字声明的变量）。
- 每个输入变量也叫顶点属性(Vertex Attribute)。我们能声明的顶点属性数量是有上限的，一般由硬件来决定，OpenGL确保至少有16个包含4分量的顶点属性可用，可查询GL_MAX_VERTEX_ATTRIBS来获取具体的上限。
- 顶点属性变量(Vertex Attribute)不能声明为数组或结构体。in 只能用于基础数据类型：int、uint、float、double和bool + 两种容器类型向量(Vector)和矩阵(Matrix)。
  #+begin_src c++
int nrAttributes;
